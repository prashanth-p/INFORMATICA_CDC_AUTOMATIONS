...
# Purpose: This Script will update the Planned Actvity activity flag in  PowerExchange Prod Config
# -----------------------------------------------------------------------------------------
# Version: 1.0.0
# -----------------------------------------------------------------------------------------
...
export region=`cat $CDC_PWX_CONFIG_FILE_PROD | grep -i $pwx_instance | cut -d":" -f10 | cut -d"=" -f2`
export pwx_server=`cat $CDC_PWX_CONFIG_FILE_PROD | grep -i $pwx_instance | cut -d":" -f5 | cut -d"=" -f2`
export pwx_erp_dl=`cat $CDC_PWX_CONFIG_FILE_PROD | grep -i $pwx_instance | cut -d":" -f12 | cut -d"=" -f2`
export updateFlag=$2
export updated_by=$3
export change=$4
export comments=$5
...
MON_FLAG_UPDATE(){
    ...
    MODULE_HEADER
    echo -e "$(timestamp): Performing Operation: Enable/Diable Flag - $pwx_instance: $updateFlag in $region"
    echo

    fileCount=`cat $CDC_PWX_CONFIG_FILE_PROD | grep -i $pwx_instance | grep -i $updateFlag | grep -v grep | wc -l`

    if [ $fileCount -gt 0 ]; then
        echo -e "$(timestamp): Planned Activity Enable/Disable Flag - $pwx_instance: $updateFlag is already updated in $region"
    ...
    backupFileName=$(echo "$CDC_PWX_CONFIG_FILE_PROD_FILENAME""_bkp_""$bkp_date")
    export backup_file=$CDC_PWX_CONFIG_FILE_PROD_BACKUP/$backupFileName
    cp $CDC_PWX_CONFIG_HOME_PROD/$CDC_PWX_CONFIG_FILE_PROD_FILENAME $backup_file
    sleep 2
    MODULE_HEADER
    ...
    echo
    echo -e "$(timestamp): Updating the Planned Activity Flag: $pwx_instance: $updateFlag in $region"
    echo
    sed -i "/$pwx_instance/Is/ACTIVITY_ENABLED=./$updateFlag/g" $CDC_PWX_CONFIG_FILE_PROD 
    echo
    echo -e "$(timestamp): The Planned Activity Flag $pwx_instance: $updateFlag has been updated in $region - PowerExchange Prod"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_PLANNED_ACTIVITY_PWX_PROD/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_PROD_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    echo -e "</tr> " >> $ALERT_FILE_NAME
    echo -e "<tr> " >> $ALERT_FILE_NAME
    echo -e "<th  width=""40%"">Planned Activity Activity Flag:</th>  " >> $ALERT_FILE_NAME
    echo -e "<td width=""60%"">$updateFlag</td>" >> $ALERT_FILE_NAME
    echo -e "</tr> " >> $ALERT_FILE_NAME
    echo -e "<tr> " >> $ALERT_FILE_NAME
    echo -e "<th width=""40%"">PowerExchange  Prod Region:</th>  " >> $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_PROD_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_PROD_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PWX_HOME_PROD_PLANNED_ACTIVITY_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_PROD_PLANNED_ACTIVITY_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - PWX Planned Activity Flag Update - PROD - $pwx_instance - $updateFlag @ $(timestamp)"
...
MODULE_HEADER() {
    ...
    echo
    echo "#############################################################"
    echo "Module : update_Planned_Actvity_activity.sh"
    echo "#############################################################"
    echo
}
