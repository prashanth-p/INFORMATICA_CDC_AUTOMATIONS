...
# Purpose: This Script will update the kafka activity flag in  PowerExchange Prod Config
# -----------------------------------------------------------------------------------------
# Version: 1.0.0
# -----------------------------------------------------------------------------------------
...
MON_FLAG_UPDATE(){
    ...
    if [ $fileCount -gt 0 ]; then
        echo -e "$(timestamp): Monitoring Enable/Disable Flag - $pwx_instance: $updateFlag is already updated in $region"
    ...
    echo
    echo -e "$(timestamp): Updating the Monitoring Flag: $pwx_instance: $updateFlag in $region"
    echo
    sed -i "/$pwx_instance/Is/MONITORING_ENABLED=./$updateFlag/g" $CDC_PWX_CONFIG_FILE_PROD 
    echo
    echo -e "$(timestamp): The Monitoring Flag $pwx_instance: $updateFlag has been updated in $region - PowerExchange Prod"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_MONITORING_PWX_PROD/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_MONITORING_PWX_PROD_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_MONITORING_PWX_PROD_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_MONITORING_PWX_PROD_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PWX_HOME_PROD_MONITORING_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_PROD_MONITORING_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - PWX Monitoring Flag Update  PROD - $pwx_instance - $region - $updateFlag @ $(timestamp)"
