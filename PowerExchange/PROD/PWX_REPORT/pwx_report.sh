...
####################################################
export tempDir=$(mktemp -d $CDC_PWX_HOME_PROD_TEMPDIR/temp.XXXXXXXXXX)
...
export ALERT_EMAIL_FILE=${ALERT_EMAILS_PWX_REPORT_PROD}/ALERT_EMAIL_FILE_$(date +"%d%b%Y_%H%M%S").html
...
write_data_to_html_file() {
    ...
    cat $ALERT_TEMPLATE_PWX_REPORT_PROD_HEADER_01 > $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PWX_REPORT_PROD_HEADER_02 >> $ALERT_EMAIL_FILE
    finalResult=""
    ...
    cat $ALERT_TEMPLATE_PWX_REPORT_PROD_FOOTER_01 >> $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PWX_REPORT_PROD_FOOTER_02 >> $ALERT_EMAIL_FILE
}
...
SEND_EMAIL_TO_TEAM() {
    ...
    if [[ ! -z "$subjectERPUnavailable" ]] || [[ ! -z "$subjectERPIssue" ]] || [[ ! -z "$subjectLoggerDown" ]] || [[ ! -z "$subListenerDown" ]] || [[ ! -z "$subERPMaintenance" ]]; then
        subject="{PowerExchange Report}{PROD}${subjectERPUnavailable}${subjectERPIssue}${subjectLoggerDown}${subERPMaintenance}${subListenerDown} - Please Check @$(timestamp)"
    else
        subject="{PowerExchange Report}{PROD} - Success - @$(timestamp)"
    fi

    echo -e "$(timestamp):\tSubject: $subject"
    echo -e "$(timestamp):\tHTML File: $ALERT_EMAIL_FILE"
    echo -e "$(timestamp):\tTo:$CDC_PWX_HOME_PROD_REPORT_TO_LIST"
    echo -e "$(timestamp):\tCC:$CDC_PWX_HOME_PROD_REPORT_CC_LIST"
    echo -e "$(timestamp):\tSending Email...!"

    (
        echo "To: $CDC_PWX_HOME_PROD_REPORT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_PROD_REPORT_CC_LIST"
