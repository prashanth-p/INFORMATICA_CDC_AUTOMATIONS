...
check_status_and_send_alert_email() {
    ...
    do
        ...
        echo -e "$(timestamp):\tPWX_ERP_REGION: $PWX_ERP_REGION"
        ...
        if [ -z "$SSH_RESULT" ]; then
            ...
            export body="$subject </br></br>- Please Check if there is an activity in ERP </br>- Please Check SSH Connectivity</br>"  
    ...
    done <  $CDC_PWX_CONFIG_FILE_PROD 
}

PREPARE_EMAIL_FOR_TEAM() {    
    alertFileName=`echo "REGULAR_MONITORING_ALERT_EMAIL_${PWX_ERP_NAME}_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_REGULAR_MONITORING_PWX_PROD/$alertFileName
    ...
    cat $ALERT_EMAILS_REGULAR_MONITORING_PWX_PROD_TEMPLATE_HEADER_01 > $ALERT_FILE_NAME
    echo "<p>$body</p></br>" >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_REGULAR_MONITORING_PWX_PROD_TEMPLATE_HEADER_02 >> $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_REGULAR_MONITORING_PWX_PROD_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_REGULAR_MONITORING_PWX_PROD_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME
...
SEND_EMAIL_TO_TEAM() {
    ...
    echo -e "$(timestamp):\tTo: $CDC_PWX_HOME_PROD_REGULAR_MONITORING_FLAG_SCRIPT_TO_LIST"
    echo -e "$(timestamp):\tCC: $CDC_PWX_HOME_PROD_REGULAR_MONITORING_FLAG_SCRIPT_CC_LIST"
    echo -e "$(timestamp):\tAlert_Email: $ALERT_FILE_NAME"
    (
        echo "To: $CDC_PWX_HOME_PROD_REGULAR_MONITORING_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_PROD_REGULAR_MONITORING_FLAG_SCRIPT_CC_LIST"
