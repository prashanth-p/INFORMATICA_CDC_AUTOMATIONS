...
# Purpose: This Script will Provide the Query Result of PWX Instance configured in Prod
# --------------------------------------------------------------------------------
# Version: 1.0.0
# --------------------------------------------------------------------------------
...
grep -i "=${INSTANCE}:" ${CDC_PWX_CONFIG_FILE_PROD} | grep -v grep > /dev/null
RET_CODE=$?
RECLINE=`grep -i "=${INSTANCE}:" ${CDC_PWX_CONFIG_FILE_PROD} | grep -v grep`
