...
# Module : display_PWX_PROD_Config.sh
...
# --------------------------------------------------------------------------------
# Usage:
#   * ./display_PWX_PROD_Config.sh
#   * ./display_PWX_PROD_Config.sh "LIVE"
# --------------------------------------------------------------------------------
# Version: 1.0.0
# --------------------------------------------------------------------------------
...
MODULE_HEADER() {
    ...
    echo
    echo "#############################################################"
    echo "Module : display_PWX_PROD_Config.sh"
    echo "#############################################################"
    echo
}
...
MODULE_HEADER
echo -e "Welcome to the PWX Configuration Query Tool - Prod"
echo
...
echo "-----------------------------------------------------------------------------------------------------------------"
NA_PROD=`cat ${CDC_PWX_CONFIG_FILE_PROD} | $grepFlag | grep "ENV=NA_PROD" | cut -d":" -f$optionFlag | cut -d"=" -f2`
EMEA_PROD=`cat ${CDC_PWX_CONFIG_FILE_PROD} | $grepFlag | grep "ENV=EMEA_PROD" | cut -d":" -f$optionFlag | cut -d"=" -f2`
ASPAC_PROD=`cat ${CDC_PWX_CONFIG_FILE_PROD} | $grepFlag | grep "ENV=ASPAC_PROD" | cut -d":" -f$optionFlag | cut -d"=" -f2`


paste <(echo "NA") <(echo "EMEA") <(echo "ASPAC") | awk -F'\t' '{ printf("%-40s %-40s %s\n", $1, $2, $3) }'
echo "-----------------------------------------------------------------------------------------------------------------"
paste <(echo "$NA_PROD") <(echo "$EMEA_PROD") <(echo "$ASPAC_PROD") | awk -F'\t' '{ printf("%-40s %-40s %s\n", $1, $2, $3) }'

echo "#################################################################################################################"
echo -e "Enter the $getInstanceBy Instance for the configuration details : \c"
read TEMP_PUB
INSTANCE=`echo ${TEMP_PUB} | tr "a-z" "A-Z"`
INSTANCE=`cat ${CDC_PWX_CONFIG_FILE_PROD} | grep -i "$INSTANCE" | cut -d":" -f1 | cut -d"=" -f2`
export $INSTANCE
