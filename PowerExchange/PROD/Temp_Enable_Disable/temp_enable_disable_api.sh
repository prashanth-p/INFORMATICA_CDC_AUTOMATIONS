...
MON_FLAG_UPDATE(){
    ...
    echo
    sed -i "/$pwx_instance/Is/TEMP_DISABLE=./$updateFlag/g" $CDC_PWX_CONFIG_FILE_PROD 
    echo
    echo -e "$(timestamp): The Temp Enable/Disable Flag $pwx_instance: $updateFlag has been updated in $region - PowerExchange Prod"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_TEMP_ENABLE_DISABLE_PWX_PROD/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_TEMP_ENABLE_DISABLE_PWX_PROD_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_TEMP_ENABLE_DISABLE_PWX_PROD_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_TEMP_ENABLE_DISABLE_PWX_PROD_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PWX_HOME_PROD_TEMP_ENABLE_DISABLE_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_PROD_TEMP_ENABLE_DISABLE_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - PWX Temp Enable/Disable Flag Update - PROD - $pwx_instance - $updateFlag @ $(timestamp)"
