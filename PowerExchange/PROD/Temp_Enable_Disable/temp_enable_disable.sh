...
# Purpose: This Script will update the Temp Enable/Disable flag in PowerExchange Prod Config
# -----------------------------------------------------------------------------------------
# Version: 1.0.0
# -----------------------------------------------------------------------------------------
...
main() {
    ...
    MODULE_HEADER
    echo -e "You have selected: Temp Enable/Disable - PowerExchange Prod"
    echo
...
PLANNED_ACTIVITY_FLAG() {
    ...
    $ALERT_EMAILS_TEMP_ENABLE_DISABLE_PWX_PROD_SCRIPT_API "$pwx_instance" "$updateFlag" "$user_email_id" "$change" "$comments" "$changeStartTime" "$changeEndTime"
...
MODULE_HEADER() {
    ...
    echo
    echo "#######################################################################"
    echo "Module : PowerExchange Prod: temp_enable_disable.sh"
    echo "#######################################################################"
    echo
}
