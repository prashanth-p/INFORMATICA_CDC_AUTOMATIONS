...
MON_FLAG_UPDATE(){
    ...
    echo
    sed -i "/$pwx_instance/Is/ACTIVITY_ENABLED=./$updateFlag/g" $CDC_PWX_CONFIG_FILE_NON_PROD 
    echo
    echo -e "$(timestamp): The Planned Activity Flag $pwx_instance: $updateFlag has been updated in $region - PowerExchange NON_PROD"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_PLANNED_ACTIVITY_PWX_NON_PROD/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_NON_PROD_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_NON_PROD_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_PLANNED_ACTIVITY_PWX_NON_PROD_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PWX_HOME_NON_PROD_PLANNED_ACTIVITY_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PWX_HOME_NON_PROD_PLANNED_ACTIVITY_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - PWX Planned Activity Flag Update - NON_PROD - $pwx_instance - $updateFlag @ $(timestamp)"
