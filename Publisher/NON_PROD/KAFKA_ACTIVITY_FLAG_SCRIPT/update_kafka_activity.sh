...
main() {
    ...
    MODULE_HEADER
    echo -e "You have selected: Enable/disable Kafka Activity Tracker - Publisher  NON_PROD"
    echo
...
KAFKA_FLAG() {
    ...
    echo
    echo -e "\t1. NA NON_PROD Region"
    echo
    echo -e "\t2. EMEA NON_PROD Region"
    echo
    echo -e "\t3. ASPAC NON_PROD Region"
    echo
    ...
    if [ "$QUERY_TYPE" == "1" ];then
        region="NA_NON_PROD"
        $ALERT_EMAILS_KAFKA_ACTIVITY_SCRIPT_API "$region" "$updateFlag" "$user_email_id" "$change" "$comments"
      
    elif [ "$QUERY_TYPE" == "2" ];then
        region="EMEA_NON_PROD"
        $ALERT_EMAILS_KAFKA_ACTIVITY_SCRIPT_API "$region" "$updateFlag" "$user_email_id" "$change" "$comments"
        
    elif [ "$QUERY_TYPE" == "3" ];then
        region="ASPAC_NON_PROD"
        $ALERT_EMAILS_KAFKA_ACTIVITY_SCRIPT_API "$region" "$updateFlag" "$user_email_id" "$change" "$comments"
    else
        echo "***Invalid option..!"
        echo "Exiting ...!"
