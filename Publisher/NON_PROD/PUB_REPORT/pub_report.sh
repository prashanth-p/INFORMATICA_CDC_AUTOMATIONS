...
####################################################
export tempDir=$(mktemp -d $CDC_PUB_HOME_NON_PROD_TEMPDIR/temp.XXXXXXXXXX)
...
export ALERT_EMAIL_FILE=${ALERT_EMAILS_PUB_REPORT_NON_PROD}/ALERT_EMAIL_FILE_$(date +"%d%b%Y_%H%M%S").html
export subjectLag=""
...
get_config_file_data() {
    ...
    do
        ...
        export PWX_ERP_ACTIVITY=`cat $CDC_PWX_CONFIG_FILE_NON_PROD | grep -i $PWX_ERP_NAME | cut -d":" -f8 |  cut -d "=" -f2`
        export PWX_TEMP_DISABLE=`cat $CDC_PWX_CONFIG_FILE_NON_PROD | grep -i $PWX_ERP_NAME | cut -d":" -f9 | cut -d "=" -f2`
        export PWX_SERVER_TIMEZONE=`cat $CDC_PWX_CONFIG_FILE_NON_PROD | grep -i $PWX_ERP_NAME | cut -d":" -f11 | cut -d "=" -f2`
        ...
        PUBLISHER_NON_PROD_EPOCH=$(TZ=${ZONE} date +"%s")
        lag="false"
        ...
        if [ -z "$checkpoint_date" ]; then
            ...
            timeDiff=$(($PUBLISHER_NON_PROD_EPOCH - $instance_epoch))
    ...
    done <  $CDC_PUB_CONFIG_FILE_NON_PROD 
}
...
write_data_to_html_file() {
    ...
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_PROD_HEADER_01 > $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_PROD_HEADER_02 >> $ALERT_EMAIL_FILE
    finalResult=""
    ...
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_PROD_FOOTER_01 >> $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_PROD_FOOTER_02 >> $ALERT_EMAIL_FILE
}
...
SEND_EMAIL_TO_TEAM() {
    ...
    if [[ ! -z "$subjectLag" ]] || [[ ! -z "$subjectPubDown" ]] || [[ ! -z "$subIssue" ]]; then
        subject="{Publisher Report}{NON_PROD}${subjectLag}${subjectPubDown}${subIssue} - Please Check @$(timestamp)"
    else
        subject="{Publisher Report}{NON_PROD} - Success - @$(timestamp)"
    fi

    echo -e "$(timestamp):\tSubject: $subject"
    echo -e "$(timestamp):\tHTML File: $ALERT_EMAIL_FILE"
    echo -e "$(timestamp):\tTo:$CDC_PUB_HOME_NON_PROD_PUB_REPORT_TO_LIST"
    echo -e "$(timestamp):\tCC:$CDC_PUB_HOME_NON_PROD_PUB_REPORT_CC_LIST"
    echo -e "$(timestamp):\tSending Email...!"

    (
        echo "To: $CDC_PUB_HOME_NON_PROD_PUB_REPORT_TO_LIST"
        echo "Cc: $CDC_PUB_HOME_NON_PROD_PUB_REPORT_CC_LIST"
