...
main() {
    ...
    MODULE_HEADER
    echo -e "You have selected: Enable/disable Monitoring - Publisher  NON_PROD"
    echo
    ...
    if [ "$QUERY_TYPE" == "Y" ]; then
        ...
        if [ `echo $QUERY_TYPE | grep -i @ITS.JNJ.COM` ]; then 
            ...
            if [ "$QUERY_TYPE" == "1" ]; then
                ...
                sleep 1
                source $DISPLAY_PUB_NON_PROD_CONFIG_SCRIPT "LIVE"
                export pub_instance=$PUB_INSTANCE    
                echo 
...
MONITORING_FLAG() {
    ...
    $ALERT_EMAILS_MONITORING_ACTIVITY_SCRIPT_API "$pub_instance" "$updateFlag" "$user_email_id" "$change" "$comments"
