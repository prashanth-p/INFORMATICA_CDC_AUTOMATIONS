...
# Purpose: This Script will update the Monitoring activity flag in  Publisher NON_PROD Config
# -----------------------------------------------------------------------------------------
# Version: 1.0.0
# -----------------------------------------------------------------------------------------
...
export region=`cat $CDC_PUB_CONFIG_FILE_NON_PROD | grep -i $pub_instance | cut -d":" -f3 | cut -d"=" -f2`
export updateFlag=$2
export updated_by=$3
export change=$4
export comments=$5
...
MON_FLAG_UPDATE(){
    ...
    fileCount=`cat $CDC_PUB_CONFIG_FILE_NON_PROD | grep -i $pub_instance | grep -i $updateFlag | grep -v grep | wc -l`
    ...
    echo
    sed -i "/$pub_instance/Is/LIVE=./$updateFlag/g" $CDC_PUB_CONFIG_FILE_NON_PROD 
    echo
    echo -e "$(timestamp): The Monitoring Flag $pub_instance: $updateFlag has been updated in $region - Publisher  NON_PROD"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_MONITORING_ACTIVITY/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PUB_HOME_NON_PROD_MONITORING_ACTIVITY_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PUB_HOME_NON_PROD_MONITORING_ACTIVITY_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - Pub Monitoring Flag Update  NON_PROD - $pub_instance - $region - $updateFlag @ $(timestamp)"
