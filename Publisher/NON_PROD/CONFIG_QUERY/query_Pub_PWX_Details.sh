...
# Module: query_Pub_PWX_Details.sh
# Purpose: This Script will Provide the Query Result of PUB  Instance configured in NON_PROD
# --------------------------------------------------------------------------------
# Usage:
#   * ./query_Pub_PWX_Details.sh "Publisher Instance Name"
#   example: ./query_Pub_PWX_Details.sh "SAP_CROADS_NON_PROD"
# --------------------------------------------------------------------------------
# Version: 1.0.0
# --------------------------------------------------------------------------------
...
grep "=${PUB_INSTANCE}:" ${CDC_PUB_CONFIG_FILE_NON_PROD} | grep -v grep > /dev/null
RET_CODE=$?
...
RECLINE=`grep "=${PUB_INSTANCE}:" ${CDC_PUB_CONFIG_FILE_NON_PROD} | grep -v grep`
...
PUB_SERVER=`echo ${RECLINE} | cut -d":" -f8 | cut -d"=" -f2`
...
PWX_SERVER_OUT=`ssh -q -o PasswordAuthentication=no -o ConnectTimeout=600 -n $PUB_USER@$PUB_SERVER $SCRIPT`
...
echo -e "\t$thirtyfive CDC Publisher NON_PROD -  Configuration" 
...
echo -e "PUB NON PROD SERVER:$PUB_SERVER" | awk -F':' '{ printf("\t| %-25s | %-60s\t|\n", $1, $2) }'
