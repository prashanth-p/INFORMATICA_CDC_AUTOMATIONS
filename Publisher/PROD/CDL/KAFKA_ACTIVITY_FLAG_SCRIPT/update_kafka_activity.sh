...
main() {
    ...
    MODULE_HEADER
    echo -e "You have selected: Enable/disable Kafka Activity Tracker - Publisher CDL Prod"
    echo
...
KAFKA_FLAG() {
    ...
    if [ "$QUERY_TYPE" == "1" ];then
        region="NA_PROD"
        $ALERT_EMAILS_KAFKA_ACTIVITY_CDL_SCRIPT_API "$region" "$updateFlag" "$user_email_id" "$change" "$comments"
