...
main() {
    ...
    MODULE_HEADER
    echo -e "You have selected: Enable/disable Monitoring - Publisher CDL Prod"
    echo
    ...
    if [ "$QUERY_TYPE" == "Y" ]; then
        ...
        if [ `echo $QUERY_TYPE | grep -i @ITS.JNJ.COM` ]; then 
            ...
            if [ "$QUERY_TYPE" == "1" ]; then
                ...
                sleep 1
                source $DISPLAY_PUB_CDL_PROD_CONFIG_SCRIPT ""
                export pub_instance=$PUB_INSTANCE    
                echo 
                echo -e "You have Selected Option 1: Enable Monitoring Flag - $pub_instance"
                sleep 1
                MONITORING_FLAG ENABLE
            elif [ "$QUERY_TYPE" == "2" ]; then
                echo -e "You have Selected Option 2: Disable Monitoring Flag"
                sleep 1
                source $DISPLAY_PUB_CDL_PROD_CONFIG_SCRIPT "LIVE"
                export pub_instance=$PUB_INSTANCE    
                echo 
                echo -e "You have Selected Option 2: Disable Monitoring Flag - $pub_instance"
                sleep 1
...
MONITORING_FLAG() {
    ...
    $ALERT_EMAILS_MONITORING_ACTIVITY_CDL_SCRIPT_API "$pub_instance" "$updateFlag" "$user_email_id" "$change" "$comments"
...
MODULE_HEADER() {
    ...
    echo
    echo "#############################################################"
    echo "Module : enable_disable_monitoring.sh"
    echo "#############################################################"
    echo
}
