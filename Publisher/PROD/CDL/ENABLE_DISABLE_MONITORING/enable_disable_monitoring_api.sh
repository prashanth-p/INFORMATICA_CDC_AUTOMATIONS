...
# Purpose: This Script will update the Monitoring activity flag in CDL Publisher Prod Config
# -----------------------------------------------------------------------------------------
# Version: 1.0.0
# -----------------------------------------------------------------------------------------
...
export region=`cat $CDC_PUB_CONFIG_FILE_CDL_PROD | grep -i $pub_instance | cut -d":" -f3 | cut -d"=" -f2`
export updateFlag=$2
export updated_by=$3
export change=$4
export comments=$5
...
MON_FLAG_UPDATE(){
    ...
    fileCount=`cat $CDC_PUB_CONFIG_FILE_CDL_PROD | grep -i $pub_instance | grep -i $updateFlag | grep -v grep | wc -l`
    ...
    backupFileName=$(echo "$CDL_CONFIG_FILE""_bkp_""$bkp_date")
    export backup_file=$CDC_PUB_CONFIG_BACKUP_HOME_PROD_CDL/$backupFileName
    cp $CDC_PUB_CONFIG_HOME_PROD_CDL/$CDL_CONFIG_FILE $backup_file
    sleep 2
    MODULE_HEADER
    ...
    echo
    sed -i "/$pub_instance/Is/LIVE=./$updateFlag/g" $CDC_PUB_CONFIG_FILE_CDL_PROD 
    echo
    echo -e "$(timestamp): The Monitoring Flag $pub_instance: $updateFlag has been updated in $region - Publisher CDL Prod"
    echo

    # Send Email to Team
    alertFileName=`echo "MONITORING_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_MONITORING_ACTIVITY_CDL/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_CDL_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    echo -e "</tr> " >> $ALERT_FILE_NAME
    echo -e "<tr> " >> $ALERT_FILE_NAME
    echo -e "<th width=""40%"">Publisher CDL Prod Region:</th>  " >> $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_CDL_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_MONITORING_ACTIVITY_CDL_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PUB_HOME_PROD_CDL_MONITORING_ACTIVITY_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PUB_HOME_PROD_CDL_MONITORING_ACTIVITY_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - Pub Monitoring Flag Update CDL PROD - $pub_instance - $region - $updateFlag @ $(timestamp)"
