...
KAFKA_FLAG_UPDATE(){
    ...
    fileCount=`cat $CDC_PUB_CONFIG_FILE_NON_CDL_PROD | grep -i $region | grep -i $updateFlag | grep -v grep | wc -l`
    ...
    echo
    sed -i "/$region/Is/KAFKA_ACTIVITY=./$updateFlag/g" $CDC_PUB_CONFIG_FILE_NON_CDL_PROD 
    echo
    echo -e "$(timestamp): The Kafka Flag $updateFlag in $region - Publisher NON_CDL Prod"
    echo

    # Send Email to Team
    alertFileName=`echo "KAFKA_UPDATE_ALERT_EMAIL_"$(date +"%d%b%Y_%H%M%S")".html"`
    export ALERT_FILE_NAME=$ALERT_EMAILS_KAFKA_ACTIVITY_NON_CDL/$alertFileName
...
PREPARE_EMAIL_FOR_TEAM() {    
    ...
    cat $ALERT_EMAILS_KAFKA_ACTIVITY_NON_CDL_TEMPLATE_HEADER > $ALERT_FILE_NAME
    ...
    cat $ALERT_EMAILS_KAFKA_ACTIVITY_NON_CDL_TEMPLATE_FOOTER_01 >> $ALERT_FILE_NAME
    echo $(timestamp) >> $ALERT_FILE_NAME
    cat $ALERT_EMAILS_KAFKA_ACTIVITY_NON_CDL_TEMPLATE_FOOTER_02 >> $ALERT_FILE_NAME

}

SEND_EMAIL_TO_TEAM() {
    (
        echo "To: $CDC_PUB_HOME_PROD_NON_CDL_KAFKA_ACTIVITY_FLAG_SCRIPT_TO_LIST"
        echo "Cc: $CDC_PUB_HOME_PROD_NON_CDL_KAFKA_ACTIVITY_FLAG_SCRIPT_CC_LIST"
        echo "Subject: Informatica CDC Automations - KAFKA FLAG Update PUB NON_CDL PROD - $region - $updateFlag @ $(timestamp)"
