...
####################################################
export tempDir=$(mktemp -d $CDC_PUB_HOME_PROD_NON_CDL_TEMPDIR/temp.XXXXXXXXXX)
...
export ALERT_EMAIL_FILE=${ALERT_EMAILS_PUB_REPORT_NON_CDL_PROD}/ALERT_EMAIL_FILE_$(date +"%d%b%Y_%H%M%S").html
export subjectLag=""
...
get_config_file_data() {
    ...
    done <  $CDC_PUB_CONFIG_FILE_NON_CDL_PROD 
}
...
write_data_to_html_file() {
    ...
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_CDL_PROD_HEADER_01 > $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_CDL_PROD_HEADER_02 >> $ALERT_EMAIL_FILE
    finalResult=""
    ...
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_CDL_PROD_FOOTER_01 >> $ALERT_EMAIL_FILE
    echo $(timestamp) >> $ALERT_EMAIL_FILE
    cat $ALERT_TEMPLATE_PUB_REPORT_NON_CDL_PROD_FOOTER_02 >> $ALERT_EMAIL_FILE
}
...
SEND_EMAIL_TO_TEAM() {
    ...
    if [[ ! -z "$subjectLag" ]] || [[ ! -z "$subjectPubDown" ]] || [[ ! -z "$subIssue" ]]; then
        subject="{Publisher Report}{NON_CDL}${subjectLag}${subjectPubDown}${subIssue} - Please Check @$(timestamp)"
    else
        subject="{Publisher Report}{NON_CDL} - Success - @$(timestamp)"
    fi

    echo -e "$(timestamp):\tSubject: $subject"
    echo -e "$(timestamp):\tHTML File: $ALERT_EMAIL_FILE"
    echo -e "$(timestamp):\tTo:$CDC_PUB_HOME_PROD_NON_CDL_PUB_REPORT_TO_LIST"
    echo -e "$(timestamp):\tCC:$CDC_PUB_HOME_PROD_NON_CDL_PUB_REPORT_CC_LIST"
    echo -e "$(timestamp):\tSending Email...!"

    (
        echo "To: $CDC_PUB_HOME_PROD_NON_CDL_PUB_REPORT_TO_LIST"
        echo "Cc: $CDC_PUB_HOME_PROD_NON_CDL_PUB_REPORT_CC_LIST"
