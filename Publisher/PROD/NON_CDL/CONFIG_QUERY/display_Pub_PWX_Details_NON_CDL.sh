...
# Module : display_Pub_PWX_Details_NON_CDL.sh
# Purpose: This Script will Provide the Query Result of PUB NON_CDL Instance configured in Prod
# --------------------------------------------------------------------------------
# Usage:
#   * ./display_Pub_PWX_Details_NON_CDL.sh 
#   * ./display_Pub_PWX_Details_NON_CDL.sh "LIVE"
# --------------------------------------------------------------------------------
# Version: 1.0.0
# --------------------------------------------------------------------------------
...
MODULE_HEADER() {
    ...
    echo
    echo "#############################################################"
    echo "Module : display_Pub_PWX_Details_NON_CDL.sh"
    echo "#############################################################"
    echo
}
...
echo "-----------------------------------------------------------------------------------------------"
NA_PROD=`cat ${CDC_PUB_CONFIG_FILE_NON_CDL_PROD} | $grepFlag | grep "ENV=NA_PROD" | cut -d":" -f1 | cut -d"=" -f2`
EMEA_PROD=`cat ${CDC_PUB_CONFIG_FILE_NON_CDL_PROD} | $grepFlag | grep "ENV=EMEA_PROD" | cut -d":" -f1 | cut -d"=" -f2`
ASPAC_PROD=`cat ${CDC_PUB_CONFIG_FILE_NON_CDL_PROD} | $grepFlag | grep "ENV=ASPAC_PROD" | cut -d":" -f1 | cut -d"=" -f2`
