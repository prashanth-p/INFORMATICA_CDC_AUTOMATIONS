...
# Module: query_Pub_PWX_Details_NON_CDL.sh
# Purpose: This Script will Provide the Query Result of PUB NON_CDL Instance configured in Prod
# --------------------------------------------------------------------------------
# Usage:
#   * ./query_Pub_PWX_Details_NON_CDL.sh "Publisher Instance Name"
#   example: ./query_Pub_PWX_Details_NON_CDL.sh "SAP_CROADS_PROD"
# --------------------------------------------------------------------------------
# Version: 1.0.0
# --------------------------------------------------------------------------------
...
grep "=${PUB_INSTANCE}:" ${CDC_PUB_CONFIG_FILE_NON_CDL_PROD} | grep -v grep > /dev/null
RET_CODE=$?
...
RECLINE=`grep "=${PUB_INSTANCE}:" ${CDC_PUB_CONFIG_FILE_NON_CDL_PROD} | grep -v grep`
...
echo -e "\t$thirtyfive CDC Publisher PROD - NON_CDL Configuration" 
...
echo -e "NON_CDL SERVER:$SERVER" | awk -F':' '{ printf("\t| %-25s | %-60s\t|\n", $1, $2) }'
